...
export function DossierCard({ dossier }: DossierCardProps) {
  ...
  const progress = getProgress(dossier.status);
  ...
  return (
    <div className="group relative bg-white rounded-2xl p-5 border border-slate-200 shadow-sm hover:shadow-md transition-all duration-200 hover:-translate-y-1">
      ...
      <Link href={`/dashboard/dossiers/${dossier.id}`} className="block">
        <h3 className="font-semibold text-slate-900 mb-1 truncate" title={dossier.title}>
          {dossier.title}
        </h3>
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-xs">
            <span className={`font-medium ${isError ? 'text-amber-600' : 'text-slate-600'}`}>
        ...
        <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between text-xs text-slate-400">
          ...
          {dossier.responsible_so && (
            <div className="flex items-center gap-1" title={`SO: ${dossier.responsible_so.email}`}>
